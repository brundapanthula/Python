def read_data(path):
    ...
    return df_allData,df_target


def data_insights(df_allData):
...
def handle_missing_values(df_allData):
...
def data_split(df_data_new):
    ...
    return df_num_data,df_nonnum_data

def check_skew(df_num_data):
    ...
    return attr,df_num_data

def check_corr(df_num_data,attr):
...
def impute_cat_attr(df_nonnum_data,df_num_data):
