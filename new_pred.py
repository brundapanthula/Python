...
def data_insights(df_train,df_test):
    ...
    pd.concat([df_train, df_train])['SalePrice'].plot(kind='hist', bins=20, title='Price frequency')
...
def model_build(y_train,train,test):
    ...
    #score = model_selection.cross_val_score(xgb_regressor, X_selected, y, cv=3, scoring=rmsle)
    #score.mean()
    #
    ##Predict test data
    ...
    #params = {'learning_rate': 0.1, 'n_estimators': 400, 'max_depth': 3, 'reg_lambda': 0, 'reg_alpha': 10}
    #xgb_regressor.set_params(**params)
    #xgb_regressor.fit(X_selected, y)
